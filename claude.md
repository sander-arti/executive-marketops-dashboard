...
- Backend: Next.js Route Handlers / API (server actions/route handlers), modulær service-layer
- Database: PostgreSQL (Supabase) + Prisma ORM  
- Auth: Supabase Auth (organisasjon/workspace-støtte)
- AI: LLM (modell konfigurerbar) + embeddings (pgvector) for RAG
- Batch/Jobs: Cron (f.eks. Vercel Cron) + kø/fan-out (valgfritt, f.eks. QStash) for månedlige genereringer
- Hosting: Vercel + Supabase

## Produktet slik det *skal fungere* (kilde-sannhet for implementasjon)

Dette prosjektet har en etablert frontend/UX. Implementasjon skal **koble funksjonalitet til eksisterende UI-patterns**, ikke redesigne.

### Navigasjon / moduler
- **Oversikt**: “leder-OS” som samler status, agenda og handlinger.
- **Bedriftsorakel (Intelligens)**: global analyse-chat på tvers av rapporter + finans.
- **Spor (rapporter):**
  - **Våre produkter** → månedlige *Produkt statusrapporter* per produkt
  - **Markedsrapport** → månedlig *Nordisk markedsrapport* (landskap)
  - **Produktradar** → månedlig *Porteføljescan* + operativ *Deal Flow & Radar* (pipeline)

### Felles “report”-pattern (skal være konsistent overalt)
Hver rapportside har:
- Tabs: **Dashboard** + **Rapportdokument**
- **Executive Summary** (headline + AI-analyse) + CTA “Les hele rapporten”
- Seksjon: **Strategiske hendelser** (events)
- **AI Assistant (Beta)** på høyre side, scoped til aktuell rapport (“Spør om rapporten…”)
- Periodeselector (typisk måned: YYYY-MM)

### Strategiske hendelser (events)
Events er “atomene” som dukker opp på tvers av rapporter (og i drawers):
- Typer: Mulighet / Risiko / Trend / Evidens / Partnerkandidat (m.fl.)
- Tags: produkt, land/marked (NO/DK/SE), “NY”
- Scoring: **Betydning**, **Risiko**, **Troverdighet** (0–10)
- Innhold i drawer-tabs: **Sammendrag / Hvorfor / Kilder / Neste steg**
- Kilder må være lenkbare og sannferdige (ingen hallusinerte kilder)

### Oversikt (ledercockpit)
Oversikt skal drives av data fra rapporter og pipeline:
- AI-oppsummering (kort situasjonsbilde) + CTA “Les Månedens Brief”
- KPI-kort (porteføljehelse, kritiske risikoer, nye muligheter)
- **Strategisk agenda** (top 3 fokusområder) med deep-links til relevante rapporter
- **Anbefalte handlinger** (Action Items) som kan markeres som utført

### Produktradar (portefølje)
To moduser:
1) **Månedlig Scan** (rapport) – executive summary + partnerkandidater (events) + rapportdokument
2) **Deal Flow & Radar** (pipeline/kanban) – stages: Nye kandidater → Til vurdering → Due diligence (+ Avvist)

### Bedriftsorakel (global chat)
- Chat-first analyseflate med “quick prompts”
- Skal kunne svare med referanser til relevante rapporter/avsnitt (RAG) når dette er implementert.

## MVP-fokus (før “nice-to-have”)
MVP er å få en **ende-til-ende** vertikal slice i prod:
1) Workspace + auth + seed data
2) Rapporter (produkt/landskap/portefølje) leses fra DB og rendres (dashboard + dokument)
3) Events med drawer-tabs + kilder
4) Oversikt med agenda + action items (done/not done)
5) Produktradar pipeline (flytte kandidat mellom stages + avvise)
6) (Valgfritt i MVP) Enkel report-scoped chat med kilder (RAG light)

## Bevisste non-goals (ikke bygg uten eksplisitt PRD-godkjenning)
- Rollebaserte dashboards/rolle-switching
- “Generer leveranser/briefs” workflow, approvals/MLR-system
- Komplett CRM
- Overgeneralisering for mange nisjer før Pharma Nordic-case er stabilt

## Compliance / sikkerhet (pharma)
- Ikke lag medisinske påstander uten tydelige kilder; svar skal være sporbare til rapporter/kilder.
- Ikke lagre eller logg hemmeligheter/PII. Ingen pasientdata i systemet (antatt).
- All AI-generert tekst som brukes operativt bør ha “kildegrunnlag” tilgjengelig (lenker/refs).
